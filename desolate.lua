...
local function httpPost(url, body)
    ...
    local ok, res = pcall(function() return game:HttpGet(url .. "?" .. HttpService:UrlEncode(payload)) end)
...
local function showKeyUI(opts)
    local A, BG, BG2 = Color3.fromRGB(0, 200, 230), Color3.fromRGB(6, 6, 10), Color3.fromRGB(14, 14, 20)
    local TX, MU = Color3.fromRGB(200, 200, 210), Color3.fromRGB(100, 100, 115)
    local ER, OK = Color3.fromRGB(255, 60, 60), Color3.fromRGB(60, 255, 130)
    ...
    ih.BackgroundColor3 = BG2; ih.BorderSizePixel = 0; ih.Parent = box
...
local function isSyncUser(plr) return syncSet[tostring(plr.UserId)] == true end
...
local function makeHeaderBtn(text, xOff, onClick)
    ...
    table.insert(headerBtns, b); return b
end
...
local function refreshModules()
    ...
    for i, mod in ipairs(list) do
        if mod.isHeader then
            ...
            nameLbl.Position = UDim2.new(0, 12, 0, 0); nameLbl.Size = UDim2.new(1, -100, 0, 30)
...
UserInputService.InputBegan:Connect(function(input, gpe)
    ...
    if capturingBind and input.UserInputType == Enum.UserInputType.Keyboard then
        if input.KeyCode == Enum.KeyCode.Escape then
            capturingBind.keybind = nil
            if capturingBtn then capturingBtn.Text = "[NONE]"; capturingBtn.TextColor3 = MUTED end
        else
            capturingBind.keybind = input.KeyCode
            if capturingBtn then capturingBtn.Text = input.KeyCode.Name; capturingBtn.TextColor3 = MUTED end
        end
        capturingBind = nil; capturingBtn = nil
    end
end)
...
local function applyStaticColors()
    ...
    watermark.BackgroundColor3 = BG2; wAccent.BackgroundColor3 = ACCENT
    wLabel.TextColor3 = TEXT; coordFrame.BackgroundColor3 = BG2
    coordLabel.TextColor3 = TEXT; targetHud.BackgroundColor3 = BG2
    thName.TextColor3 = TEXT; thInfo.TextColor3 = MUTED
    if kbFrame then kbFrame.BackgroundColor3 = BG2 end
    if kbTitle then kbTitle.TextColor3 = ACCENT end
    if kbList then kbList.TextColor3 = TEXT end
    buildCrosshair()
    if drawFovFrame then drawFovStroke.Color = ACCENT end
    mobileBtn.BackgroundColor3 = BG2; mobileBtn.TextColor3 = ACCENT
    mStroke.Color = ACCENT
    for _, b in ipairs(headerBtns) do b.BackgroundColor3 = BG4; b.TextColor3 = TEXT end
end
...
local function makeDraggable(frame, name, dx, dy)
    ...
    local hint = Instance.new("UIStroke"); hint.Color = ACCENT; hint.Thickness = 1; hint.Transparency = 0.75; hint.Parent = frame
end
...
wAccent.Size = UDim2.new(0, 3, 1, 0); wAccent.BackgroundColor3 = ACCENT; wAccent.BorderSizePixel = 0; wAccent.Parent = watermark
...
coordLabel.TextColor3 = TEXT; coordLabel.Text = "X: -- Y: -- Z: --"; coordLabel.Parent = coordFrame
...
crosshair.Name = "Crosshair"; crosshair.AnchorPoint = Vector2.new(0.5, 0.5)
crosshair.Position = UDim2.new(0.5, 0, 0.5, 0); crosshair.Size = UDim2.new(0, 20, 0, 20)
crosshair.BackgroundTransparency = 1; crosshair.Visible = false; crosshair.Parent = hudGui
local chMode = "circle"
function buildCrosshair()
    ...
    elseif chMode == "cross" then
        for _, data in ipairs({
            ...
            local b = Instance.new("Frame"); b.Size = data[1]; b.Position = data[2]
            b.BackgroundColor3 = ACCENT; b.BorderSizePixel = 0; b.Parent = crosshair
        end
    end
end
buildCrosshair()
...
kbFrame.Position = UDim2.new(0, 400, 0, 10)
kbFrame.BackgroundColor3 = BG2; kbFrame.BackgroundTransparency = 0.2
kbFrame.BorderSizePixel = 0; kbFrame.Visible = false; kbFrame.Parent = hudGui
Instance.new("UICorner", kbFrame).CornerRadius = UDim.new(0, 8)
local kbAccent = Instance.new("Frame")
kbAccent.Size = UDim2.new(0, 3, 1, 0); kbAccent.BackgroundColor3 = ACCENT; kbAccent.BorderSizePixel = 0; kbAccent.Parent = kbFrame
Instance.new("UICorner", kbAccent).CornerRadius = UDim.new(0, 8)
local kbTitle = Instance.new("TextLabel")
kbTitle.BackgroundTransparency = 1; kbTitle.Position = UDim2.new(0, 10, 0, 4)
kbTitle.Size = UDim2.new(1, -14, 0, 16); kbTitle.Font = FONT; kbTitle.TextSize = 11
kbTitle.TextXAlignment = Enum.TextXAlignment.Left; kbTitle.TextColor3 = ACCENT
kbTitle.Text = "KEYBINDS"; kbTitle.Parent = kbFrame
local kbList = Instance.new("TextLabel")
kbList.BackgroundTransparency = 1; kbList.Position = UDim2.new(0, 10, 0, 22)
kbList.Size = UDim2.new(1, -14, 1, -26); kbList.Font = FONT; kbList.TextSize = 11
kbList.TextXAlignment = Enum.TextXAlignment.Left; kbList.TextYAlignment = Enum.TextYAlignment.Top
kbList.TextColor3 = TEXT; kbList.TextWrapped = true; kbList.Text = ""; kbList.Parent = kbFrame

MOD.keyBinds.actions.onToggle = function(on) kbFrame.Visible = on end
makeDraggable(kbFrame, "keybinds", 400, 10)
...
RunService.Heartbeat:Connect(function()
    ...
    if wasOnGround and not onG then
...
local function ensureTrailEmitter()
    ...
    local hrp = CACHE.hrp; if not hrp then return nil end
    local att = hrp:FindFirstChild("DesolateTrailAtt")
    if not att then
        att = Instance.new("Attachment"); att.Name = "DesolateTrailAtt"
        att.Position = Vector3.new(0, -1.5, 0); att.Parent = hrp
    end
    local em = att:FindFirstChild("Emitter")
    if not em then
        em = Instance.new("ParticleEmitter"); em.Name = "Emitter"
        ...
        em.Lifetime = NumberRange.new(0.6); em.Rate = 0; em.Speed = NumberRange.new(0); em.Parent = att
    end
    trailEmitter = em; return em
end

RunService.Heartbeat:Connect(function(dt)
    if not MOD.trails.enabled or not CACHE.hrp then return end
    trailAccum += dt
    if trailAccum >= 0.06 then
        trailAccum = 0
        local em = ensureTrailEmitter()
        if em then em.Color = ColorSequence.new(ACCENT); em:Emit(2) end
    end
end)
...
local function buildParticles()
    ...
    local hrp = CACHE.hrp; if not hrp or not MOD.particles.enabled then return end
    ...
    att.Name = "DesolateFallingEmitter"; att.Position = Vector3.new(0, 25, 0); att.Parent = hrp
    local em = Instance.new("ParticleEmitter")
    em.Name = "Emitter"; em.Texture = "rbxassetid://243098098"
    ...
    em.Lifetime = NumberRange.new(3); em.Speed = NumberRange.new(speed)
    em.Direction = Vector3.new(0, -1, 0); em.SpreadAngle = Vector2.new(20, 20)
    em.Acceleration = Vector3.new(0, -25, 0); em.Rotation = NumberRange.new(0, 360)
    em.RotSpeed = NumberRange.new(-90, 90); em.Rate = rate
    em.LightEmission = 0.6; em.LightInfluence = 0; em.Parent = att
end
...
MOD.particles.actions.onSliderChange = function(idx, v)
    ...
    local att = hrp:FindFirstChild("DesolateFallingEmitter"); if not att then return end
    ...
    elseif idx == 3 then em.Size = NumberSequence.new({ NumberSequenceKeypoint.new(0, v / 5), NumberSequenceKeypoint.new(1, v / 20) }) end
    em.Color = ColorSequence.new(ACCENT)
end
player.CharacterAdded:Connect(function() task.wait(1); refreshCharacterCache(); if MOD.particles.enabled then buildParticles() end end)
task.spawn(function()
    while gui.Parent do
        task.wait(1)
        if MOD.particles.enabled then
            if not CACHE.hrp then refreshCharacterCache() end
            if CACHE.hrp and not CACHE.hrp:FindFirstChild("DesolateFallingEmitter") then buildParticles() end
        end
    end
end)
...
local function ensureSkyObject()
    if not customSkyObj or not customSkyObj.Parent then
        customSkyObj = Instance.new("Sky"); customSkyObj.Name = "DesolateCustomSky"
...
MOD.customSky.actions.onToggle = function(on)
    if on then applySkyPreset(currentSkyPreset); Lighting.ClockTime = MOD.customSky.slider.value
    else
...
MOD.fog.actions.onChange = function(v) if not MOD.fog.enabled then return end; Lighting.FogStart = 0; Lighting.FogEnd = v end
MOD.skyPreset.actions.onToggle = function(on)
    if not on then return end
    currentSkyPreset = math.floor(MOD.skyPreset.slider.value)
    if MOD.customSky.enabled then applySkyPreset(currentSkyPreset); Lighting.ClockTime = MOD.customSky.slider.value end
end
MOD.skyPreset.actions.onChange = function(v)
    currentSkyPreset = math.floor(v)
    if not MOD.customSky.enabled then return end
    applySkyPreset(currentSkyPreset); Lighting.ClockTime = MOD.customSky.slider.value
end
...
targetHud.Size = UDim2.new(0, 220, 0, 70); targetHud.Position = UDim2.new(0.5, 40, 0.5, 40)
targetHud.BackgroundColor3 = BG2; targetHud.BackgroundTransparency = 0.1
targetHud.BorderSizePixel = 0; targetHud.Visible = false; targetHud.Parent = hudGui
Instance.new("UICorner", targetHud).CornerRadius = UDim.new(0, 8)
local thName = Instance.new("TextLabel")
thName.BackgroundTransparency = 1; thName.Position = UDim2.new(0, 10, 0, 4); thName.Size = UDim2.new(1, -14, 0, 18)
thName.Font = FONT; thName.TextSize = 13; thName.TextColor3 = TEXT
thName.TextXAlignment = Enum.TextXAlignment.Left; thName.Text = "Target"; thName.Parent = targetHud
local thInfo = Instance.new("TextLabel")
thInfo.BackgroundTransparency = 1; thInfo.Position = UDim2.new(0, 10, 0, 22); thInfo.Size = UDim2.new(1, -14, 0, 14)
thInfo.Font = FONT; thInfo.TextSize = 11; thInfo.TextColor3 = MUTED
thInfo.TextXAlignment = Enum.TextXAlignment.Left; thInfo.Text = "HP: -- | --m"; thInfo.Parent = targetHud
local thBarBg = Instance.new("Frame")
thBarBg.Position = UDim2.new(0, 10, 1, -16); thBarBg.Size = UDim2.new(1, -20, 0, 6)
thBarBg.BackgroundColor3 = BG4; thBarBg.BorderSizePixel = 0; thBarBg.Parent = targetHud
Instance.new("UICorner", thBarBg).CornerRadius = UDim.new(0, 6)
local thBar = Instance.new("Frame")
thBar.Size = UDim2.new(1, 0, 1, 0); thBar.BackgroundColor3 = ERROR
...
-- FPS
-- =========================================================
local fps = 0; local frames = 0; local t0 = tick()
...
task.spawn(function()
    ...
    while gui.Parent do
        ...
        if MOD.watermark.enabled then
            ...
            wLabel.Text = string.format("Desolate %s | FPS: %d | PING: %d", player.Name, fps, ping)
        end

        if MOD.coords.enabled and CACHE.hrp then
            coordLabel.Text = string.format("X: %.1f Y: %.1f Z: %.1f", CACHE.hrp.Position.X, CACHE.hrp.Position.Y, CACHE.hrp.Position.Z)
        end
        ...
        if MOD.keyBinds.enabled then
            ...
            kbList.Text = #lines > 0 and table.concat(lines, "\n") or "no active binds"
        end
        ...
        if tickCount % 2 == 0 then
            ...
            if MOD.targetHUD.enabled then
                ...
                if plr and hum then
                    ...
                    thBar.BackgroundColor3 = pct > 0.5 and OK or pct > 0.25 and Color3.fromRGB(255, 220, 100) or ERROR
                else
...
Players.PlayerAdded:Connect(function(plr)
    plr.CharacterAdded:Connect(function(char)
        ...
        hum.Died:Connect(function()
            if not MOD.killEffect.enabled then return end
            local myHrp = CACHE.hrp; local hrp = char:FindFirstChild("HumanoidRootPart")
            if not myHrp or not hrp then return end
            if (myHrp.Position - hrp.Position).Magnitude < 60 then showKillEffect(plr.Name) end
        end)
    end)
end)
        ...
        if cat ~= "Keybind" then
            for _, mod in ipairs(list) do
                if not mod.isHeader and mod.keybind and input.KeyCode == mod.keybind then
                    mod.enabled = not mod.enabled
                    if mod.actions.onToggle then pcall(mod.actions.onToggle, mod.enabled) end
                    if currentCat == cat then refreshModules() end
                end
            end
        end
    end
...
mobileBtn.Size = UDim2.new(0, 44, 0, 44); mobileBtn.Position = UDim2.new(0, 10, 0.5, -22)
mobileBtn.BackgroundColor3 = BG2; mobileBtn.TextColor3 = ACCENT
...
local mStroke = Instance.new("UIStroke"); mStroke.Color = ACCENT; mStroke.Thickness = 1; mStroke.Transparency = 0.4; mStroke.Parent = mobileBtn
mobileBtn.MouseButton1Click:Connect(function() main.Visible = not main.Visible end)
main:GetPropertyChangedSignal("Visible"):Connect(function() mobileBtn.Visible = not main.Visible end)
